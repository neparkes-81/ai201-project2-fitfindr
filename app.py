...
def handle_query(user_query: str, wardrobe_choice: str) -> tuple[str, str, str]:
    ...
    # 5. Format the selected listing into a readable block and return it
    #    alongside the outfit suggestion and the fit card.
    item = session["selected_item"]
    price = item.get("price")
    price_text = f"${price:g}" if isinstance(price, (int, float)) else "n/a"
    listing_text = (
        f"{item.get('title', 'Unknown item')}\n"
        f"Price: {price_text}  |  Platform: {item.get('platform', 'n/a')}\n"
